...
<div class="panel panel-form">
    <div class="panel-heading">Soal</div>
        <div class="panel-body" style="background-color:white">
            <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#createModal" data-id="{{$test->id_test}}" hidden>Tambah Soal</button>
            <!-- Klo jumlah soal yang dibuat >= jumlah soal test yang udah ditentuin -->
            @if($status && $soals->total() >= $test->jumlah_soal)
            <a href="{{url('/admin/finishcreate/'.$test->materi->id_mat)}}" type="button" class="btn btn-success pull-right"><i class="fa fa-check-circle" aria-hidden="true"></i> Finish</a>
            <br><br>
            <!-- Klo jumlah soal yang dibuat >= jumlah soal test yang udah ditentuin -->
            @elseif(!$status && $soals->total() >= $test->jumlah_soal)
            <a href="{{url('/admin/tipetes/'.Hashids::encode($test->materi->id_mat,$status))}}" type="button" class="btn btn-success pull-right"><i class="fa fa-check-circle" aria-hidden="true"></i> Finish</a>
            <br><br>
            @else
            <!-- Klo jumlah soal yang dibuat < jumlah soal test yang udah ditentuin -->
...
<script type="text/javascript" src="http://js.nicedit.com/nicEdit-latest.js"></script> 
    <script type="text/javascript">
        bkLib.onDomLoaded(function() {
            new nicEditor({buttonList : ['bold','italic','underline','subscript','superscript']}).panelInstance('area1');
            new nicEditor({buttonList : ['bold','italic','underline','subscript','superscript']}).panelInstance('area2');
            new nicEditor({buttonList : ['bold','italic','underline','subscript','superscript']}).panelInstance('area3');
            new nicEditor({buttonList : ['bold','italic','underline','subscript','superscript']}).panelInstance('area4');
            new nicEditor({buttonList : ['bold','italic','underline','subscript','superscript']}).panelInstance('area5');
            new nicEditor({buttonList : ['bold','italic','underline','subscript','superscript']}).panelInstance('area6');
            new nicEditor({buttonList : ['bold','italic','underline','subscript','superscript']}).panelInstance('area7');
            new nicEditor({buttonList : ['bold','italic','underline','subscript','superscript']}).panelInstance('area8');
            new nicEditor({buttonList : ['bold','italic','underline','subscript','superscript']}).panelInstance('area9');
            new nicEditor({buttonList : ['bold','italic','underline','subscript','superscript']}).panelInstance('area10');
        });
    </script>
