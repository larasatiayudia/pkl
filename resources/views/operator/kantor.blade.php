      ...
      <div class="table-responsive">
        <table id="mytable" class="table table-bordred table-striped">
          ...
          <tbody>
            @if(count($kantors))
              ...
              <td> 
                <p data-placement="top" data-toggle="tooltip" title="Edit">
                  <button class="btn btn-primary  open_modal" data-title="Edit" data-toggle="modal" data-target="#editKantor" data-value="{{$kantor->nama_kantor}}" data-tipe="{{$kantor->tipe}}" data-id="{{$kantor->id_kantor}}" ata-level="{{$kantor->level}}">
           ...
           @else
           <tr>
             <td colspan="7">Belum ada data kantor</td>
           </tr>
           @endif
          </tbody>
        </table>
        ...
        <table id="mytable" class="table table-bordred table-striped">
          ...
          @if(count($tipekantor))
           @foreach($tipekantor as $tipe)
              ...
              <td> 
                <p data-placement="top" data-toggle="tooltip" title="Edit">
                  <button class="btn btn-primary  open_modal" data-title="Edit" data-toggle="modal" data-target="#editTipe" data-grup="{{$tipe->id_grup}}" data-tipe="{{$tipe->tipe_kantor}}" data-id="{{$tipe->id_tipe}}" data-level="{{$tipe->level}}">
              ...
              <td>
                <form id="deleteform" class="delete" role="form" method="POST" action="{{route('operator.deleteTipe',['id'=>$tipe->id_tipe])}}" enctype="multipart/form-data">
                  <p data-placement="top" data-toggle="tooltip" title="Delete"> 
                    ...
                    <button class="btn btn-danger " type="button" id="delete" data-name="{{$tipe->tipe_kantor}}" data-table="Tipe Kantor">
                      <span class="glyphicon glyphicon-trash"></span>
                    </button>
                  </p>
                  {{csrf_field()}}
           ...
           @endforeach
           @else
           <tr><td colspan="5">Belum ada data tipe kantor</td></tr>
           @endif
          </tbody>
        </table>    
      </div>
      ...
      <div class="modal-body">
        <form role= "form" id="addKantor" method="POST" action="{{route('operator.addKantor')}}" enctype="multipart/form-data">
          ...
          <div class="col-md-8">
            ...
            <select id="superkantor" name="superkantor" class="selectpicker" data-live-search="true">
      ...
      <form role= "form" id="edit" method="POST" action="{{route('operator.editKantor')}}" enctype="multipart/form-data">
        ...
        <div class="col-md-4">
         <label class="control-label">Tipe Kantor</label>
          ...
          <div class="col-md-8">
            <label class="control-label">Koordinasi Kantor</label><br>
            <select id="superkantor" name="superkantor" class="selectpicker" data-live-search="true">
            </select>
          </div>
          </div>
          ...
          <div class="form-group">
            <label class="control-label">Nama Kantor</label>
            <input id="kantor" name="kantor" type="text" required="required" class="form-control" placeholder="Masukkan Nama Kantor" value=""/>
          </div>
        </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-danger" data-dismiss="modal">Close</button>
...
<div class="modal fade" id="addTipe" tabindex="-1" role="dialog">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form role= "form" id="" method="POST" action="{{route('operator.addTipe')}}" enctype="multipart/form-data">
          <div class="form-group">
          ...
          <div class="col-md-4">
            ...
            <h6 style="margin-top: 2px">Note: Level 0-9 melambangkan hirarki tertinggi-terendah</h6>
          </div>
          <div class="col-md-8">
      ...
      <div class="modal-body">
        <form role= "form" id="editTipe" method="POST" action="{{route('operator.editTipe')}}" enctype="multipart/form-data">
         <div class="form-group">
          ...
          <div class="form-group">
            <label class="control-label">Tipe : </label>
            <input id="tipe" type="text" name="tipe" required="required" class="form-control" placeholder="Masukkan Tipe/Status Kantor" />
          </div>
          <div class="form-group">
             <label class="control-label">Level : </label>
            <select id="level" name="level" class="form-control">
